...
const AddJobs = () => {
    const handleAddJobs = (e) => {
        ...
        const { min, max, currency, ...newJobData } = formObj
        console.log(newJobData);
        newJobData.salaryRange = { min, max, currency }

        const requirements = newJobData.requirements;
        const requirementsDirty = requirements.split(',')
        ...
        axios.post('http://localhost:3000/jobs', newJobData).then(res => console.log(res))
    }
    return (
        <div>
            <h2>Please add a job</h2>
            <form onSubmit={handleAddJobs} className='w-5/12 mx-auto'>
